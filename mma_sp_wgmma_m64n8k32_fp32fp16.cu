...
[[maybe_unused]] constexpr int kSparseK = kBlockK / 2;
[[maybe_unused]] constexpr int kMetaBytes = kBlockK / 8;

using MmaOp = SM90::GMMA::SPARSE::GMMA_64x8x32_F32F16F16_SS<
    SM90::GMMA::Major::K,
    SM90::GMMA::Major::MN>;
...
inline float to_float(ElementA x) {
  return static_cast<float>(x);
}

inline ElementA to_half(float x) {
  return ElementA{x};
}
...
void init_structured_sparse_a(
  ...
  a_dense.assign(m * k, ElementA{0.0f});
  for (int row = 0; row < m; ++row) {
    for (int group = 0; group < k / 4; ++group) {
      ...
      a_dense[base + i0] = to_half(v0);
      a_dense[base + i1] = to_half(v1);
    }
  }
}

void init_pattern_sparse_a(std::vector<ElementA>& a_dense, int m, int k) {
  a_dense.assign(m * k, ElementA{0.0f});
  for (int row = 0; row < m; ++row) {
    for (int group = 0; group < k / 4; ++group) {
      int base = row * k + group * 4;
      ...
      a_dense[base + 0] = to_half(v0);
      a_dense[base + 2] = to_half(v1);
    }
  }
}

void init_random_b(std::vector<ElementB>& b, int k, int n, std::mt19937& gen) {
  std::uniform_real_distribution<float> dist(-1.0f, 1.0f);
  b.resize(k * n);
...
void init_pattern_b(std::vector<ElementB>& b, int k, int n) {
  b.resize(k * n);
  for (int kk = 0; kk < k; ++kk) {
    for (int nn = 0; nn < n; ++nn) {
...
void compress_structured_sparse_a(
    ...
    int m,
    int k) {
  a_sparse.assign(m * (k / 2), ElementA{0.0f});
  ...
  for (int row = 0; row < m; ++row) {
    for (int group = 0; group < k / 4; ++group) {
      ...
      int nz = 0;
      for (int i = 0; i < 4; ++i) {
        if (to_float(a_dense[base + i]) != 0.0f) {
          if (nz < 2) {
            idxs[nz] = i;
          }
          ++nz;
...
void cpu_gemm_ref(
  ...
  for (int row = 0; row < m; ++row) {
    for (int col = 0; col < n; ++col) {
      float accum = 0.0f;
      for (int kk = 0; kk < k; ++kk) {
        accum += to_float(a_dense[row * k + kk]) * to_float(b[kk * n + col]);
      }
      c[row * n + col] = accum;
    }
  }
...
__global__ void wgmma_sp_kernel(
    ElementA const* __restrict__ a_sparse,
    ElementB const* __restrict__ b,
  ...
  for (int k_tile = 0; k_tile < k; k_tile += kBlockK) {
    for (int idx = tid; idx < kBlockM * kSparseK; idx += blockDim.x) {
      int row = idx / kSparseK;
      int col = idx % kSparseK;
      sAraw(row, col) = a_sparse[(block_row + row) * (k / 2) + (k_tile / 2) + col];
    }

    for (int idx = tid; idx < kBlockN * kBlockK; idx += blockDim.x) {
      int col = idx / kBlockK;
      int kk = idx % kBlockK;
      sB(col, kk) = b[(k_tile + kk) * n + (block_col + col)];
    }

    for (int idx = tid; idx < kBlockM * kMetaBytes; idx += blockDim.x) {
...
bool run_case(
  ...
  if ((m % kBlockM) != 0 || (n % kBlockN) != 0 || (k % kBlockK) != 0) {
    std::cerr << tag << " requires M%64==0, N%8==0, K%32==0" << std::endl;
    return false;
  }
  ...
  wgmma_sp_kernel<<<grid, block>>>(d_a, d_b, d_e, d_c, m, n, k);
  ...
  int errors = verify_result(tag, c_gpu, c_ref, m, n);
...
int main() {
  ...
  std::cout << "Sparse WGMMA demo: wgmma.mma_async.sp.sync.aligned.m64n8k32.f32.f16.f16 on device "
            << device << " (" << props.name << ")" << std::endl;

  std::mt19937 gen(1234);
  bool ok = true;
  ok &= run_case("pattern_64x8x32", 64, 8, 32, true, gen);
  ok &= run_case("random_64x8x128", 64, 8, 128, false, gen);
  ok &= run_case("random_128x32x256", 128, 32, 256, false, gen);

...
struct SharedStorage_fp16 {
  alignas(128) half smem_A[SparseConfig_fp16<BlockN>::kASmemElements];
  alignas(128) half smem_B[BlockN * kBlockK_fp16];
  alignas(128) uint8_t smem_E[SparseConfig_fp16<BlockN>::kESmemBytes];
};
...
#define FP16_OUT_FLOATS_64                                                     \
  FP16_OUT_FLOATS_32, "+f"(d[32]), "+f"(d[33]), "+f"(d[34]),          \
...
#define FP16_OUT_FLOATS_128                                                    \
  FP16_OUT_FLOATS_64, "+f"(d[64]), "+f"(d[65]), "+f"(d[66]),          \
...
template <int BlockN, typename WgmmaFunc>
__global__ void wgmma_sp_raw_kernel_impl(
    half const* __restrict__ a_sparse,
    half const* __restrict__ b,
    uint8_t const* __restrict__ e_bytes,
    float* __restrict__ c,
    int n,
    int k,
    WgmmaFunc wgmma_func) {
#if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900) && defined(__CUDA_ARCH_FEAT_SM90_ALL)
  __shared__ SharedStorage_fp16<BlockN> shared;

  constexpr int kAccRegs = (BlockN == 256) ? 128 : 64;
  int tid = threadIdx.x;
  ...
  uint64_t desc_a = make_gmma_desc(
      ...
      SparseConfig_fp16<BlockN>::kADescLeading,
      SparseConfig_fp16<BlockN>::kADescStride);
  uint64_t desc_b = make_gmma_desc(
      smem_ptr_as_uint(shared.smem_B),
      BDescParams_fp16<BlockN>::kLeading,
      BDescParams_fp16<BlockN>::kStride);
...
#pragma unroll
  ...
  for (int k_tile = 0; k_tile < k; k_tile += kBlockK_fp16) {
    ...
    for (int idx = tid; idx < BlockN * kBlockK_fp16; idx += kThreads) {
      int col = idx / kBlockK_fp16;
      int kk = idx % kBlockK_fp16;
      shared.smem_B[smem_b_index_fp16<BlockN>(col, kk)] =
    ...
    warpgroup_arrive();
    wgmma_func(desc_a, desc_b, accum, e, scale_d);
  ...
  store_accum_fp16<BlockN>(accum, c, n, block_row, block_col, tid);
#endif
}
